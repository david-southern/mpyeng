...
/// <Acknowledgement>
/// This code is copied from https://github.com/dickiepotter/Vector/blob/master/RP.Math.Vector3/Vector3.cs
/// </Acknowledgement>
/// </summary>
...
public struct Vector3 : IEquatable<Vector3>
{
    ...
    /// This results in the need for pre-initialization initialization of the Vector3 components to 0 
    ...
    public double Magnitude
    {
        get
        {
            return Math.Sqrt(this.SumComponentSquares());
        }
    }
    ...
    /// <exception cref="System.ArgumentException">
    /// Thrown if the requested index does not correspond to one of the three components x,y,z
    /// </exception>
    ...
    /// Thrown when the normalization of a zero magnitude vector is attempted.
    /// </exception>
    /// <exception cref="NormalizeVectorException">
    /// Thrown when the normalization of a NaN magnitude vector is attempted.
    /// </exception>
    /// <remarks>
    ...
    /// <returns>Normalized special case vectors, NaN or the original vector</returns>
    private static Vector3 NormalizeSpecialCasesOrOriginal(Vector3 v1)
    {
        if (double.IsInfinity(v1.Magnitude))
        {
    ...
    {
        /* If the two vectors are equal then the angle is 0 even if there are infinite components to the vectors.
    ...
    /// <acknowledgement>Provided by Poggel, Steven Buraje in comments on CodeProject ( http://www.codeproject.com/Articles/17425/A-Vector-Type-for-C )</acknowledgement>
    ...
    public static double SumComponentSquares(Vector3 v1)
    {
    ...
    /// <returns>The sum of the Vectors X^2, Y^2 and Z^2 components</returns>
    /// <implementation>
    /// <see cref="SumComponentSquares(Vector3)"/>
    /// The Components.SumComponentSquares(Vector3) function has been used to prevent code duplication
    /// </implementation>
    public double SumComponentSquares()
    {
        return SumComponentSquares(this);
    }
    ...
    /// Uses the <see cref="NormalizeOrDefault(Vector3)"/> function in the check to avoid code duplication and to include special case of infinite values.
    /// Of the three flavors of AlmostEquals that have been implemented the absolute value tolerance is the only one 
    ...
    /// <implementation>
    /// Uses the <see cref="DotProduct(Vector3)"/> function in the check to avoid code duplication.
    /// Uses the <see cref="NormalizeOrDefault(Vector3)"/> function in the check to avoid code duplication and to include special cased of infinite values.
    /// </implementation>
    ...
    /// <returns>Truth if the two vectors are perpendicular</returns>
    /// <implementation>
    /// Uses the <see cref="IsPerpendicular(Vector3, Vector3)"/> function in the check to avoid code duplication.
    /// </implementation>
    ...
    /// <returns>Truth if the two vectors are perpendicular</returns>
    /// <implementation>
    /// Uses the <see cref="IsPerpendicular(Vector3, Vector3, double)"/> function in the check to avoid code duplication.
    /// Of the three flavors of AlmostEquals that have been implemented the absolute value tolerance is the only one 
    /// that makes senses when comparing to 0. If we later implement relative and/or ulp tolerance, this method would not benefit from the overloads.
    /// </implementation>
    ...
    /// used when an attempt is made to normalize a vector with magnitude NaN.
    /// </summary>
