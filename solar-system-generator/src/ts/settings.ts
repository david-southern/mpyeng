...
export class SSGSettings {
    ...
    private constructor() {
        ...
        Logger.info(SSGSystemFilter.Initialization, 'Constructing SSGSettings');
    }

    public SystemRoot$ = new BehaviorSubject<CelestialObject | undefined>(
        undefined
    );
    public get SystemRoot() {
        return this.SystemRoot$.value;
    }
    public set SystemRoot(value) {
        this.SystemRoot$.next(value);
    }
    ...
    public DirectionalLightPosition$ = new BehaviorSubject(
    ...
    public AnimationTimeScale$ = combineLatest([
        this.AnimationSpeed$,
        this.MaxAnimationSpeedScale$,
    ]).pipe(
    ...
    public AnimationTimeScaleHuman$ = this.AnimationTimeScale$.pipe(
        map((timeScale) =>
            Utils.FloatEQ(timeScale, 0)
                ? 'paused'
                : `${Utils.HumanTime(timeScale)} per second`
        )
    );
    ...
    public get GridType() {
        Logger.info(
            ...
            `GridType$: Returning value ${this.GridType$.value}`
        );
