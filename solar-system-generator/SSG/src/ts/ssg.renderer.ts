...
export class SSGRenderer {
    ...
    public initialize(canvasDivId: string, systemTimeDivId: string) {
        ...
        this.composer.addPass(new PP.EffectPass(this.camera, new PP.ColorDepthEffect({
            bits: 32
        })));
    ...
    public render(solarSystem: CelestialObject, newSettings: SSGSettings) {
        ...
        SettingsManager.subscribeSettings((settings: SSGSettings) => {
            ...
            if (settings.IncludeDirectionalLight) {
                ...
                Utils.setPosition(this.directionalLight, settings.DirectionalLightPosition);
            }
        });
    ...
    private buildSolarSystem(rootObject: CelestialObject): THREE.Group {
        ...
        if (majorAxis > 0) {
            ...
            if (rootObject.OrbitColor !== 'none') {
                const orbitObject = Utils.buildOrbitalMesh(0, 0, 0, orbitCurve, rootObject.OrbitColor ?? DEFAULT_ORBITAL_COLOR);
