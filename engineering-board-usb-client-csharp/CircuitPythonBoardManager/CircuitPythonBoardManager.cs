...
public class CircuitPythonBoardManager
{
    ...
    public static void ScanSerialPorts()
    {
        ...
        if (portInfos.Count > 0)
        {
            ...
            {
                Log.Information($"  Port: {portInfo}");
                Log.Information($"        Raw Name: {portInfo.Name}");
                Log.Information($"        Raw DeviceID: {portInfo.DeviceID}");
            }
        }
        else
        {
    ...
    public static CircuitPythonBoard? FindBoard(ClientType clientType)
    {
        ...
        Log.Information($"No {clientType} board found");
        return null;
    }
}