...
class SerialProtocolHandler:
    ...
    def reset_port(self) -> None:
        """
        Resets both input and output buffers of the serial port. This method acquires the
        communication lock before resetting the buffers to ensure thread safety. The reset clears
        any pending data in both the input and output buffers.
        """
        ...
        with self.communication_lock:
            try:
                ...
                if response != self.SER_PROTO_INIT_RESPONSE:
                    logger.debug(
                        f"{self} is not a SerProto client: Invalid init response: {response}"
                    )
                    return False
                ...
                logger.error(
                    f"{self} is not a SerProto client: Timeout during establish_connection"
                )
            except Exception as ex:
                logger.error(
                    f"{self} is not a SerProto client: Unknown exception in establish_connection: {str(ex)}",
                    exc_info=True,
                )
    ...
    def query_engine_power(self) -> list[EnginePower]:
        logger.debug("Sending QueryEnginePower")
        return [
    ...
    def query_transformer_power(self) -> list[TransformerPower]:
        logger.debug("Sending QueryTransformerPower")
        return [
    ...
    def query_system_power(self) -> list[SystemPower]:
        logger.debug("Sending QuerySystemPower")
        return [
    ...
    def query_list(self, protocol_token: str) -> list:
        """
        Query a list from the serial device using a protocol token.
        This method sends a query command to the device and expects a JSON-formatted list in response.
        The response is expected to be in the format: <protocol_token><response_token><delimiter><json_list>
        Args:
            protocol_token (str): The protocol token to use in the query command.
        Returns:
            list: The parsed JSON list from the device response. Returns empty list if:
                - Response format is invalid
                - JSON parsing fails
                - Serial communication times out
                - Any other communication error occurs
        Raises:
        ...
        with self.communication_lock:
            try:
                ...
                logger.debug(f"DBG: QueryList: Sending: {protocol_command}")
                self.port.write((protocol_command + "\n").encode("utf-8"))
                response = self.port.readline().decode("utf-8").strip()
                logger.debug(f"DBG: QueryList: Received: {response}")
                ...
                if (
                    ...
                    logger.error(
                        f"SerProto {self.port_name}({self.board_name}): Invalid query list response: {response}"
                    )
                    return empty_result
                ...
                try:
                    ...
                    logger.error(
                        f"SerProto {self.port_name}({self.board_name}): Invalid query list response format: {response}: {str(ex)}",
                        exc_info=True,
                    )
                    return empty_result
            except serial.SerialTimeoutException:
                logger.error(
                    f"SerProto {self.port_name}({self.board_name}): Timeout during query list"
                )
    ...
    def set_list(self, protocol_token: str, data: list) -> str:
        """
        Sets a list value in the protocol by sending a request through the serial port.
        Args:
            protocol_token (str): The protocol token identifier
            data (list): The list data to be sent
        Returns:
            str: Response from the serial port. Returns either:
                - SER_PROTO_OK if successful
                - SER_PROTO_ERR with optional error message if failed
        Raises:
            No exceptions are raised, errors are handled internally and logged.
        """
        with self.communication_lock:
            try:
                ...
                if response != self.SER_PROTO_OK and not response.startswith(
                    ...
                    logger.error(f"{self}: Invalid set list response: {response}")
                    return self.SER_PROTO_ERR

                return response
            except serial.SerialTimeoutException:
                logger.error(f"{self}: Timeout during set list")
                return self.SER_PROTO_ERR + ":Timeout"
            except Exception as ex:
                logger.error(
                    f"{self}: Unknown exception in set list: {str(ex)}",
                    exc_info=True,
                )
                return self.SER_PROTO_ERR + f":{str(ex)}"
    ...
    def dispose(self) -> None:
        logger.debug(f"{self}: Disposing")
        self.is_valid = False

        if self.port and self.port.is_open:
            logger.debug(f"{self}: Closing SerialPort")
            self.port.close()
        logger.debug(f"{self}: Disposing SerialPort")
        del self.port
