    ...
    public static List<SerialPortInfo> GetPortInformation(bool showRawData = false)
    {
        ...
        foreach (ManagementObject property in Ports)
        {
            ...
            if (name != null && name.ToString().Contains("USB") && name.ToString().Contains("COM"))
            {
                ...
                // Only include ports for which we can resolve the SerNo, all other ports are not CircuitPython boards
                ...
                if (showRawData)
                {
                    Log.Information($"Raw Serial Data:");
                    Log.Information($"        Name: {name}");
                    Log.Information($"        DeviceID: {portInfo?.DeviceID ?? "<null>"}");
                }
            }
        }
