...
public class SerialProtocolHandler : IDisposable
{
    ...
    public const string SER_PROTO_SWITCHBOARD_QUERY = "SP_SWB_Q";
    public const string SER_PROTO_SWITCHBOARD_RESPONSE = "SP_SWB_R";
    public const string SER_PROTO_SET_DISPLAY_VALUE = "SP_DSP_VAL";
    public const string SER_PROTO_SET_GRID_LEVEL = "SP_GRID_LEVEL";
    ...
    public List<List<string>>? QuerySwitchboard()
    {
        lock (CommunicationLock)
        {
            try
            {
                ResetPort();
                port.WriteLine(SER_PROTO_SWITCHBOARD_QUERY);
                string response = port.ReadLine();

                string[] responseParts = response.Split(SER_PROTO_RESPONSE_DELIMITER);

                if (responseParts.Length != 2 || responseParts[0] != SER_PROTO_SWITCHBOARD_RESPONSE)
                {
                    Logger.Error($"SerProto {PortName}({BoardName}): Invalid switchboard response: {response}");
                    return null;
                }

                string? jsonData = JsonConvert.DeserializeObject<string>(responseParts[1]);

                if(jsonData == null)
                {
                    Logger.Error($"{this}: Invalid switchboard response format: {response}");
                    return null;
                }

                List<List<string>>? retval = JsonConvert.DeserializeObject<List<List<string>>>(jsonData);

                if (retval == null)
                {
                    Logger.Error($"{this}: Invalid switchboard jsonData format: {response}");
                    return null;
                }
            ...
            {
                Logger.Error($"SerProto {PortName}({BoardName}): Timeout during switchboard query");
            }
    ...
    public List<EngCard> QueryCards()
    {
        ...
        lock (CommunicationLock)
        {
            try
            {
                ...
                if (cardIds == null)
                {
                    Logger.Error($"{this}: Invalid card query response format: {response}");
                    return retval;
                }
    ...
    public string SetPowerGridLevel(List<PowerGrid> data)
    {
        lock (CommunicationLock)
        {
            try
            {
                ...
                string request = SER_PROTO_SET_GRID_LEVEL + SER_PROTO_RESPONSE_DELIMITER
                    + JsonConvert.SerializeObject(data);
                port.WriteLine(request);
                string response = port.ReadLine();
                ...
                if (response != SER_PROTO_OK && !response.StartsWith(SER_PROTO_ERR))
                {
                    Logger.Error($"{this}: Invalid set power grid response: {response}");
                    return SER_PROTO_ERR;
                }
            ...
            {
                Logger.Error($"{this}: Timeout during power grid set");
                return SER_PROTO_ERR + ":Timeout";
            }
        }
    }
