    ...
    public class SimplexNoise : INoise
    {
      ...
      138,236,205,93,222,114,67,29,24,72,243,141,128,195,78,66,215,61,156,180
        ...
        public float GetNoise(float x)
        {
            ...
            // REMOVED: but we want to match PRMan's 1D noise, so we scale it down some more.
            return 0.395f * (n0 + n1);

        }

        // 2D simplex noise
        public double GetNoise(double dx, double dy)
        {
            float x = (float)dx;
            float y = (float)dy;
            ...
            return (float)Math.Clamp(47.9f * (n0 + n1 + n2) + 0.01, -1, 1);
        }

        public double NormalizedNoise(double x, double y, double z)
        {
            return GetNoise(x, y, z) / 2 + 0.5f;
        }

        // 3D simplex noise
        public double GetNoise(double dx, double dy, double dz)
        {
            float x = (float)dx;
            float y = (float)dy;
            float z = (float)dz;
            ...
            return Math.Clamp(32.0f * (n0 + n1 + n2 + n3), -1, 1); // TODO: The scale factor is preliminary!
        }
