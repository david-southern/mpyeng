...
class PowerCard:
    ...
    def InitializePowerCards(cls):
        ...
        _ALL_POWER_CARDS.append(PowerCard(2,  80, 1.058, 0.028, cardName='C02'))
        ...
        _ALL_POWER_CARDS.append(PowerCard(8,  60, 0.301, 0.049, cardName='C08'))
        _ALL_POWER_CARDS.append(PowerCard(9,  30, 1.734, 0.014))
        _ALL_POWER_CARDS.append(PowerCard(10, 30, 1.202, 0.025, cardName='C10'))
        ...
        _ALL_POWER_CARDS.append(PowerCard(26, 20, 1.263, 0.025, cardName='C30'))
        ...
        _ALL_POWER_CARDS.append(PowerCard(30, 20, 1.502, 0.019, cardName='C30'))
    ...
    def CheckSensorMatch(cls, sensorVoltage) -> "PowerCard":
        matches = [card for card in _ALL_POWER_CARDS if card.MatchesVoltage(sensorVoltage)]
        if(len(matches) > 1):
    ...
    def __init__(self, uid, requiredPower, cardVoltage, dVoltage, cardName = None):
        ...
        if(len(duplicates) > 0):
            raise Exception(f"PowerCard({uid}/{cardName}): duplicate UID with card {duplicates[0].UID}/{duplicates[0].CardName}")

        self.uid = uid
        self.requiredPower = int(requiredPower)
        self.cardVoltage = float(cardVoltage)
        self.dVoltage = float(dVoltage)
        self.cardName = cardName if cardName else f"C#{uid}"

        if self.cardVoltage <= 0 or self.cardVoltage >= 5:
            raise ValueError(f"PowerCard({uid}): Invalid cardVoltage: {cardVoltage}")
        if self.dVoltage <= 0 or self.dVoltage >= 5:
            raise ValueError(f"PowerCard({uid}): Invalid dVoltage: {dVoltage}")

        overlaps = [card for card in _ALL_POWER_CARDS if card.MaxVoltage >= self.MinVoltage and card.MinVoltage <= self.MaxVoltage]

        if(len(overlaps) > 0):
            raise Exception(f"PowerCard({uid}/{cardName}): voltage range overlaps with card {overlaps[0].UID}/{overlaps[0].CardName}")
    ...
    @property
    def CardVoltage(self):
        return self.cardVoltage

    @property
    def DeltaVoltage(self):
        return self.dVoltage

    @property
    def MinVoltage(self):
        return self.cardVoltage - self.dVoltage

    @property
    def MaxVoltage(self):
        return self.cardVoltage + self.dVoltage

    def MatchesVoltage(self, sensorVoltage):
        return abs(self.cardVoltage - sensorVoltage) < self.dVoltage
