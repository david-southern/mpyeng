...
{
    color("white")
    downcube([strip_length, led_width, led_height]);

    color("black")
    translate([0, 0, -led_height])
    downcube([strip_length, strip_width, strip_height]);
}
...
module card_frame()
{
    ...
    difference()
    {
        ...
        translate([card_width / 2 - difference_fudge, 0, 0])
        rotate([-90, 0, 90])
        led_strip(card_height);
        ...
        // Mounting mags
        color("silver")
        translate([0, 0, -(card_frame_depth - mag_thickness + difference_fudge) / 2])
        {
